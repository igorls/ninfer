...
def run_mtp_real(model_dir: str, ple_dir: str, token_list, draft_steps: int, dump_root: str):
    ...
    position t, over ALL prompt slots as one multi-token forward (this is what seeds
    the draft KV in vLLM). Then `draft_steps` chained steps from the last slot, each
    fed the previous step's mtp_multi_hidden and its own argmax token, position +1
    on all three MRoPE rows."""
    ...
    # Chained draft steps from the last slot.
    multi = stages["mtp_multi_hidden"][:, -1:, :]
    tok = int(stages["mtp_draft_tokens"][0, -1].item())
