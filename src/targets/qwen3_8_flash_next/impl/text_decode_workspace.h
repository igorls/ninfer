...
FlashNextTextDecodeWorkspace allocate_flash_next_text_decode_workspace(Arena& arena,
    ...
    ws.ple_injection   = arena.alloc(DType::BF16, {10'240, batch}, 256);
    ws.selected_blocks = arena.alloc(DType::I32, {512, batch}, 256);
    ws.selected_counts = arena.alloc(DType::I32, {batch}, 256);
    ws.hyper_scratch   = allocate_flash_next_hyper_workspace(arena, batch);
    return ws;
}
