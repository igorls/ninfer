...
void test_logical_page_lifecycle_and_residency() {
    // Plan a small 8-group pool
    ...
    // Allocate host memory as backing buffer for physical pool metadata test
    const std::size_t backing_bytes = builder.finish(256);
    std::vector<std::byte> backing_storage(backing_bytes, std::byte{0});
    const ninfer::DeviceSpan backing{backing_storage.data(), backing_storage.size()};
...
void test_address_space_store() {
    ...
    rt::KVAddressSpaceStore addresses(pages, physical_tables, 4, 2);
    ...
    // Materialize 65 tokens (spans across 2 64-token pages)
    addresses.materialize_to_tokens(*addr, 65);
...
int main() {
    std::cout << "Running ninfer_logical_kv_store_test...\n";
    test_logical_page_lifecycle_and_residency();
    test_address_space_store();
